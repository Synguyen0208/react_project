...
class Order_Details extends Component {
    ...
    callAPI=(form)=>{
        this.a.callAPI("http://localhost/react-backend/order.php", form, "POST");
        setTimeout(() => {
    ...
    componentDidMount() {
        ...
        if(id!=null){
            axios({
                method:'GET',
                data: form,
                url: 'http://localhost/react-backend/order_details.php?id='+id,      
            })
            .then(response => {
    ...
    render() {
        ...
        if (this.state.data.length > 0) {
            table = <DataTables  delete="none" update="none" data={this.state.data} />;
        }
        return (
            <div>
                <div class="card mb-4">
