...
class Layout_right extends Component {
    ...
    setData=(name, id)=>{
        switch (name) {
            case "product":
                this.setState({data: <Admin_product/>})
                break;
            case "company":
                this.setState({data: <Admin_company />})
                break;
            case "admin_account":
                this.setState({data: <Admin_Acc_Ad />})
                break;
            case "admin_user":
                this.setState({data: <Admin_Acc_User />})
                break;
            case "admin_order":
                this.setState({data: <Admin_Order click={this.setData}/>})
                break;
            case "admin_shipping":
                this.setState({data: <Admin_shipping/>})
                break;
            case "order_detail":
                this.setState({data: <Order_Details id={id}/>})
                break;
            default:
                break;
        }
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div className="container-fluid">
                        ...
                        <div className="row">
                            <Card name="Product management" click={this.setData} link="product" color="#3366ff"/>
                            <Card name="Supply partner management" click={this.setData} link="company" color="#6633cc"/>
                            <Card name="Account admin management" click={this.setData} link="admin_account" color="##660033"/>
                            <Card name="Account user management" click={this.setData} link="admin_user" color="#FF6666"/>
                            <Card name="Order management" click={this.setData} link="admin_order" color="#00CCCC"/>
                            <Card name="Shipping partners" click={this.setData} link="admin_shipping" color="#FF3333"/>
                        </div>
                        {this.state.data}
                        {/* <Switch>
                            <Route exact path="/">
